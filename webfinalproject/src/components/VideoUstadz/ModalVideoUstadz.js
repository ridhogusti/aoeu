...
import { fetchVideo, createVideo, updateVideo } from '../../actions/video';
import VideoForm from './VideoForm';

class ModalVideoUstadz extends Component {
  state = {
    errors: '',
  }
  // constructor(props) {
  ...
  componentDidMount = () => {
    ...
    if (match.params.id) {
      this.props.fetchVideo(match.params.id);
    }
  }
  ...
  saveVideo = (args) => {
    ...
    if (args._id) {
      ...
      this.props.updateVideo(fd).then(
        // (res) => this.props.history.push('/'),
        (res) => this.props.history.push(`/${akses.username}/video`),
      ...
      const fd = new FormData();
      console.log(args);
      if (args.title != null & args.video != null) {
        fd.append('title', args.title);
        fd.append('video', args.video);
        this.props.createVideo(fd).then(
          (res) => this.props.history.push(`/${akses.username}/video`),
  ...
  render() {
    return (
      <div>
        {this.state.errors}
        <VideoForm errorss={this.state.errors} saveVideo={this.saveVideo} editvideo={this.props.editvideo} />
      </div>
    );
  }
}
