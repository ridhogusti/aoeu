...
export function getCountArtikel(args) {
  ...
  return dispatch => axios.get(`http://maridakwah.com:3000/artikels/${args}/count`)
    .then(data => dispatch({ type: COUNT_ARTIKEL, data }));
}
export function fetchArtikels(args) {
  console.log(args, 'dari action artikel');
  return dispatch => axios.get(`http://maridakwah.com:3000/artikels/${args}`)
    .then(data => dispatch({ type: FETCH_ARTIKELS, data }));
}

export function fetchAllArtikels() {
  return dispatch => axios.get('http://maridakwah.com:3000/artikels/')
    .then(data => dispatch({ type: FETCH_ALL_ARTIKELS, data }));
}

export function fetchArtikel(args) {
  return dispatch => axios.get(`http://maridakwah.com:3000/artikels/ambil/${args}`)
    .then(data => dispatch({ type: FETCH_ARTIKEL, data }));
}
export function createArtikel(data) {
  return dispatch => axios.post('http://maridakwah.com:3000/artikels', data).then(res => {
...
export function updateArtikel(data) {
  return dispatch => axios.put(`http://maridakwah.com:3000/artikels/${data.get('_id')}`, data).then(res => {
    console.log(res, 'respone dari create');
    const IsData = res.data;
...
export function deleteArtikel(id) {
  ...
  return dispatch => axios.delete(`http://maridakwah.com:3000/artikels/${id}`).then(res => {
    console.log(res, 'respone dari create');
...
export function limitArtikelUmum(limit) {
  ...
  return dispatch => axios.get(`http://maridakwah.com:3000/artikels/${limit}/umum`).then(res => {
    console.log(res, 'respone dari create');
    const IsData = res.data;
...
export function limitArtikel(limit, username) {
  ...
  return dispatch => axios.get(`http://maridakwah.com:3000/artikels/${limit}/${username}`).then(res => {
    console.log(res, 'respone dari create');
    const IsData = res.data;
    dispatch({ type: LIMIT_ARTIKEL, IsData });
  });
