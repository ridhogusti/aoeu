...
import { fetchVideosUstdaz } from '../../actions/video';
...
class VideoDetail extends Component {
  ...
  async componentWillMount() {
    await this.props.fetchVideosUstdaz(this.props.navigation.getParam('video').author.username);
  }
  goToCard = (video) => {
      ...
      <View>
        ...
        <View>
          <VideoPlayer
            ...
            video={{ uri: `http://10.0.3.2:3000/videos/${this.props.navigation.getParam('video').video}` }}
            videoWidth={100}
            videoHeight={50}
          />
          <Button
        ...
        >
          ...
          >
            ...
            >{this.props.navigation.getParam('video').title}</H3>
            <Text
              style={{
                color: '#BBBBBB',
              }}
            >{this.props.navigation.getParam('video').author.name}</Text>
            <Text
              style={{
                color: '#BBBBBB',
              }}
            >{this.props.navigation.getParam('video').createdAt}</Text>
          </View>
        </Animatable.View>
      </View>
  ...
  render() {
    return (
      <ScrollView style={styles.container}>
        ...
        {
          this.props.videodetailustadz.map((video, i) => 
            // const nomor = i;
            (
              <Content key={video._id}>
                <TouchableHighlight
...
function mapStateToProps(state) {
  return {
    ...
    videodetailustadz: state.videodetailustadz.data,
  };
}

export default connect(mapStateToProps, { fetchVideosUstdaz })(VideoDetail);
