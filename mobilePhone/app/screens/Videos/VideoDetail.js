...
class VideoDetail extends Component {
  ...
  async componentWillMount() {
    await this.props.fetchVideos(this.props.navigation.getParam('video').author.username);
  }
      ...
      <View>
        ...
        <View>
          <VideoPlayer
            ...
            video={{ uri: `http://localhost:3000/videos/${this.props.navigation.getParam('video').video}` }}
            videoWidth={100}
            videoHeight={50}
          />
          <Text>{this.props.navigation.getParam('video').video}</Text>
          <Button
  ...
  render() {
    return (
      <ScrollView style={styles.container}>
        ...
        {
          this.props.videodetail.map((video, i) => 
            // const nomor = i;
            (
...
function mapStateToProps(state) {
  return {
    ...
    videodetail: state.videodetail.data,
  };
}
